...
function initializeSecureConnection() {
    ...
    try {
        // Initialize client instance dynamically
        supabase = supabaseClient.createClient(SUPABASE_URL, completeSecretKey);
...
async function testAndSyncDatabase() {
    try {
        ...
        if (error) throw error; // If the token combination is wrong, Supabase drops an error here
        
        // If successful, save state data cache, unlock interface canvas
